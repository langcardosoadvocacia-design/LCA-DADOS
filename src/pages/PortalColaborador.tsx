...
import { LogIn, DollarSign, Calendar, TrendingUp, History, User, LogOut, FileText, CheckCircle2, Circle, ListTodo, Plus } from 'lucide-react';
...
export function PortalColaborador() {
  ...
  const carregarDadosSessao = useCallback(async (idColab: string | number) => {
    ...
    try {
        ...
        const [portalRes, transRes, tarefasRes, eventosRes] = await Promise.all([
          supabase.from('colaboradores').select('*').eq('id', idColab).single(),
  ...
  const toggleTarefa = async (id: string, currentStatus: boolean) => {
    try {
      ...
      setDemandas(demandas.map(t => t.id === id ? { ...t, concluida: !currentStatus } : t));
    } catch (error: any) {
    ...
    return (
      <div style={{ 
        ...
        <motion.div 
          ...
          style={{ width: '100%', maxWidth: '400px', padding: '2.5rem', textAlign: 'center', background: 'white', borderRadius: '16px', border: '1px solid rgba(0,0,0,0.08)' }}
        >
          <div style={{ 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem' }}>
              ...
              <input 
                ...
                style={{ 
                  ...
                  color: '#1a1a2e', padding: '0.75rem', borderRadius: '8px', width: '100%'
                }}
              />
            </div>
            <button 
  ...
  return (
    <motion.div
      ...
      style={{ minHeight: '100vh', background: '#f8fafc', padding: '1rem' }}
    >
      <div style={{ maxWidth: '1000px', margin: '0 auto', width: '100%' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', background: 'white', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--color-border)' }}>
          ...
          <button onClick={logout} className="btn-outline" style={{ color: 'var(--color-danger)', border: 'none' }}>
            <LogOut size={20} />
          </button>
        </header>
